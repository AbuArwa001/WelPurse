...
import uuid
from welpurse_v2.forms.event import EventForm
from welpurse_v2.utils import login_required
from welpurse_v2.routes import app_routes
import requests
import logging
from welpurse_v2.utils import get_current_user
from welpurse_v2.routes.helper_funtions import (
    fetch_welfares,
    fetch_a_member,
    start_donation,
)
...
from welpurse_v2.routes import app_routes
from welpurse_v2.forms.donation_req import DonationRequestForm
...
def donation_request():
    ...
    return render_template(
        "donation.html", current_user=current_user, title=title, form=form
    )
...
def donation_request_view():
    ...
    if form_event.validate_on_submit():
        ...
        data = {
            ...
            "event_date": form_event.event_date.data.strftime(
                "%Y-%m-%d %H:%M:%S"
            ),
            "start_date": form_event.start_date.data.strftime(
                "%Y-%m-%d %H:%M:%S"
            ),
        ...
        try:
            ...
            if res.status_code == 201:
                ...
                if start_donation(
                    headers=headers, request_id=form.request_id.data
                ):
                    flash("Data passed successfully", "success")
                    return redirect(
                        url_for("app_routes.donation_request_view")
                    )
...
def approve_donation(request_id):
    ...
    url = (
        f"http://127.0.0.1:5001/api/v1/donation-requests/{request_id}/approve"
    )
