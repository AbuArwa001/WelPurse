...
# WALLET TRANSACTIONS
@app_views.route(
    "/wallets_transact/<wallet_id>", methods=["GET"], strict_slashes=False
)
...
# CHECKOUT
@app_views.route(
    "/wallets/checkout/<wallet_id>", methods=["POST"], strict_slashes=False
)
...
# INTERNAL TRANSFERS
@app_views.route(
    "/wallets/<wallet_id>/transfer/<destination_id>",
    methods=["POST"],
...
def internal_wallet_transfer(wallet_id, destination_id):
    ...
    response = service.wallets.intra_transfer(
        origin_wallet_id, destination_wallet_id, amount, narrative
    )
    print(response)
...
# EXTERNAL TRANSFER
@app_views.route(
    "/wallets/<wallet_id>/transfer/", methods=["POST"], strict_slashes=False
)
def external_wallet_transfer(wallet_id):
    ...
    transactions = [
        {"name": "Awesome Customer 1", "account": account1, "amount": 10},
        {"name": "Awesome Customer 2", "account": account, "amount": 10},
    ]

    response = service.transfer.mpesa(
        wallet_id=wallet_id, currency="KES", transactions=transactions
    )
    print(response)
...
#  TRANSACTION STATUS
@app_views.route(
    "/transaction/<tracking_id>", methods=["GET"], strict_slashes=False
)
