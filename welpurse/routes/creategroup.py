...
from .helper_funtions import fetch_members, join_group, add_role, fetch_a_role, fetch_roles
import requests
import logging
...
def create_welfare_group():
    ...
    member_choices = [(member.get('id'), member.get('email')) for member in members]
                    ...
                    for role in roles: 
                        if value and key == role.get('name'):
                            ...
                            if join_group(headers=headers, data=data):
                                create_role = add_role(headers=headers, member_id=value, role_id=role.get('id'))
                                logging.info("SUCCESS: Added member %s of role_id %s", value, role.get('id'))
                            else:
                                flash("Failed creating a group", "danger")
                                logging.error("FAILED: Could not add member %s", value)
                                return redirect(url_for('app_routes.create_welfare_group'))
    ...
    return render_template('creategroup.html', current_user=current_user, form=form, title=title)
