...
# PROCESS PAYEMENTS
def process_payment(current_user, wallet, email, phone, amount, event_id, type_id, contr_type):
...
def update_database(current_user, wallet, amount, event_id, type_id, contr_type):
    ...
    try:
        async with httpx.AsyncClient() as client:
            ...
            if welfare_res.status_code == 200:
                ...
                updated_wallet = await update_wallet(headers, wal_id, data_wallet)
                if not updated_wallet:
                    return {"error": "Unable to update wallet"}
                return {"welfare": welfare, "event": event, "updated_wallet": updated_wallet}
            else:
